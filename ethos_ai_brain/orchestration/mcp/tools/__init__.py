...
def discover_tools() -> List[Dict[str, Any]]:
    """
    Discover tools using UV-first approach.
    
    Priority:
    1. UV-managed tools (pyproject.toml + tool.py)
    2. Legacy tools (tool.py only) - for backward compatibility
    """
    # Try UV-first discovery
    try:
        from .uv_first_discovery import UVFirstToolManager
        tools_dir = Path(__file__).parent
        manager = UVFirstToolManager(tools_dir)
        uv_tools = manager.discover_tools()
        
        logger.info(f"UV-first discovery attempt: found {len(uv_tools)} tools")
        for tool in uv_tools:
            logger.info(f"  - {tool['name']}: uv_managed={tool.get('uv_managed', False)}")
        
        if uv_tools:
            logger.info(f"Using UV-first discovery: found {len(uv_tools)} tools")
            return uv_tools
        else:
            logger.warning("UV-first discovery found no tools, falling back to legacy")
    except Exception as e:
        logger.warning(f"UV-first discovery failed, falling back to legacy: {e}")
        import traceback
        logger.warning(f"UV-first discovery traceback: {traceback.format_exc()}")
