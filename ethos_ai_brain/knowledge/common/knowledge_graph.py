...
class KnowledgeGraph(KnowledgeSystem):
    """
    ...
    def __init__(self, graph_id: str, graph_type: str = "generic",
        ...
        self._graph_id = graph_id or f"{graph_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

        # Create NetworkX graph via composition
        self.graph = nx.DiGraph() if directed else nx.Graph()

        # Cross-graph connections (external references)
        self.external_references: Dict[
            str, Dict[str, Any]] = {}  # {node_id: {target_graph: graph_id, target_node: node_id, relationship: str}}
        ...
        # Update base metadata with graph-specific info
        self.update_metadata({
            "graph_type": graph_type,
            "graph_id": self._graph_id,
        })

        logger.info(f"KnowledgeGraph created: {self._graph_id} ({graph_type})")
    ...
    def resolve_external_reference(self, local_node: str, target_graph: str, target_node: str) -> Optional[
        Dict[str, Any]]:
        """
                ...
                if target_graph_id not in connected_graphs:
                    ...
                    if target_graph:
                        connected_graphs[target_graph_id] = {
                            "graph_type": target_graph.graph_type,
    ...
    def _assign_layered_3d_positions(self) -> None:
        ...
        if self.graph_type == 'mission':
            graph_z_base = 10.0
        elif self.graph_type == 'tactical':
        ...
        elif self.graph_type == 'local_rag':
            graph_z_base = 2.0
        elif self.graph_type == 'cloud_rag':
            graph_z_base = 0.0
        else:
            graph_z_base = 5.0
    ...
    def _update_modified_time(self):
        """Update the last modified timestamp - override base class method"""
    ...
    def export_to_json(self, include_metadata: bool = True) -> Dict[str, Any]:
        ...
        logger.info(
            f"Exported graph {self.graph_id} to JSON: {len(graph_data['nodes'])} nodes, {len(graph_data['links'])} edges")
        return export_data
    ...
    def __str__(self) -> str:
        ...
        return f"KnowledgeGraph(id={self.graph_id}, type={self.graph_type}, nodes={len(self.graph.nodes())}, edges={len(self.graph.edges())})"
    ...
    def query(self, query: str, context: Dict = None, **filters) -> List[KnowledgeResult]:
        """Query nodes in this knowledge graph"""
        try:
            results = []
                ...
                if matches_filters:
                    ...
                    if query.lower() in node_text.lower():
                        result = KnowledgeResult(
                            ...
                            knowledge_id=f"{self.graph_id}:{node_id}",
                            knowledge_type="graph_node",
                            confidence=1.0,
                            metadata=node_attrs,
                            context_path=[self.graph_id, node_id]
                        )
    ...
    def add_knowledge(self, content: Any, metadata: Dict = None) -> str:
        """Add a node to this knowledge graph"""
        try:
            ...
            success = self.add_node(node_id, **(metadata or {}))
            return f"{self.graph_id}:{node_id}" if success else ""
    ...
    def get_related(self, knowledge_id: str, relationship_type: str = "semantic") -> List[KnowledgeResult]:
        """Find related nodes in this graph"""
        try:
            # Extract node_id from knowledge_id
            if ":" in knowledge_id:
                _, node_id = knowledge_id.split(":", 1)
            else:
            ...
            if node_id in self.graph:
                ...
                for neighbor_id in neighbors:
                    neighbor_attrs = self.get_node_attributes(neighbor_id)
                    
                    result = KnowledgeResult(
                        content=f"Related Node: {neighbor_id}",
                        knowledge_id=f"{self.graph_id}:{neighbor_id}",
                        knowledge_type="graph_node",
                        confidence=0.8,
                        metadata=neighbor_attrs,
                        context_path=[self.graph_id, neighbor_id]
                    )
                    results.append(result)
    ...
    def get_capabilities(self) -> Dict[str, bool]:
        """Return capabilities of basic knowledge graph"""
        return {
            "semantic_search": True,
            "relationship_traversal": True,
            "cross_layer_queries": False,
            "temporal_queries": False,
            "multi_dimensional_clustering": False,
            "3d_positioning": True,
            "physics_simulation": False,
            "cross_graph_analysis": False
        }
    
    def get_components(self) -> List[str]:
        """Get components (nodes for basic graphs)"""
        return self.nodes()
    
    def get_relationships(self) -> List[Tuple[str, str, Dict]]:
        """Get relationships (edges with metadata)"""
        relationships = []
        for source, target, attrs in self.graph.edges(data=True):
            relationships.append((source, target, dict(attrs)))
