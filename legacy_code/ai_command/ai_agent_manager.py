"""
AI Agent Manager for dtOS Command System
Handles lifecycle management of AI agents
"""
...
@dataclass
class AIAgent:
    """AI Agent data structure with ZMQ server"""
    id: str
...
class AIAgentManager:
    ...
    def _initialize_agent_graphs(self):
        ...
        communication_patterns = [
            ...
            (AgentType.TECHNICAL_ARCHITECT, AgentType.DOCUMENTATION_SPECIALIST, {'frequency': 'medium', 'type': 'collaboration'})
        ]
    ...
    def get_available_agents(self, agent_type: Optional[AgentType] = None) -> List[AIAgent]:
    ...
    def analyze_agent_network_metrics(self) -> Dict[str, Any]:
        ...
        if len(self.agent_dependency_graph) > 0:
            metrics['dependency_analysis'] = {
                ...
                'dependency_depth': len(nx.dag_longest_path(self.agent_dependency_graph)) if nx.is_directed_acyclic_graph(self.agent_dependency_graph) else 0,
                'independent_agents': [node for node in self.agent_dependency_graph.nodes() if self.agent_dependency_graph.in_degree(node) == 0],
                'most_dependent': max(self.agent_dependency_graph.nodes(), key=lambda x: self.agent_dependency_graph.in_degree(x), default=None)
            }
        ...
        if len(self.capability_graph) > 0:
            metrics['capability_analysis'] = {
                ...
                'most_versatile_agent': max(self.capability_graph.nodes(), key=lambda x: self.capability_graph.degree(x), default=None),
                'capability_clusters': len(list(nx.connected_components(self.capability_graph))),
                'average_capability_overlap': sum(data.get('overlap_score', 0) for _, _, data in self.capability_graph.edges(data=True)) / max(len(self.capability_graph.edges()), 1)
            }
        ...
        if len(self.communication_graph) > 0:
            metrics['communication_analysis'] = {
                ...
                'communication_hub': max(self.communication_graph.nodes(), key=lambda x: self.communication_graph.degree(x), default=None),
                'average_path_length': nx.average_shortest_path_length(self.communication_graph) if nx.is_connected(self.communication_graph.to_undirected()) else 0,
