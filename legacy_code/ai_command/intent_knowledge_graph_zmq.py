...
class IntentKnowledgeGraph(KnowledgeGraph):
    ...
    @classmethod
    def get_objectives_prompt(cls, user_input: str, domain_context: str = "unknown") -> str:
        """Generate focused prompt for extracting objectives only"""
        return f"""
FOCUSED TASK: Extract ONLY the objectives from this user request.

USER REQUEST: "{user_input}"
DOMAIN CONTEXT: {domain_context}

Return JSON array of objectives with:
- id: unique identifier
- name: clear objective name
- description: detailed description
- priority: "critical"|"high"|"medium"|"low"
- measurable: true|false
- measurement_method: how to measure success (if measurable)
- target_value: numeric target (if applicable)
- unit: measurement unit (if applicable)
- complexity: "low"|"medium"|"high"|"very_high"

Focus ONLY on what the user wants to achieve. Return only the JSON array.
"""

    @classmethod
    def get_constraints_prompt(cls, user_input: str, domain_context: str = "unknown") -> str:
        """Generate focused prompt for extracting constraints only"""
        return f"""
FOCUSED TASK: Extract ONLY the constraints from this user request.

USER REQUEST: "{user_input}"
DOMAIN CONTEXT: {domain_context}

Return JSON array of constraints with:
- id: unique identifier
...
- time_pressure: "low"|"moderate"|"tight"|"unrealistic" (for timeline constraints)
- budget_breakdown: object with category breakdowns (for budget constraints)

Focus ONLY on limitations and restrictions. Return only the JSON array.
"""

    @classmethod
    def get_stakeholders_prompt(cls, user_input: str, domain_context: str = "unknown", objectives_context: str = "") -> str:
        """Generate focused prompt for extracting stakeholders only"""
        return f"""
FOCUSED TASK: Extract ONLY the stakeholders from this user request.

USER REQUEST: "{user_input}"
DOMAIN CONTEXT: {domain_context}
OBJECTIVES CONTEXT: {objectives_context}

Return JSON array of stakeholders with:
- id: unique identifier
...
- communication_preferences: object with preferences (optional)

Based on the objectives and domain, identify who would be decision makers, influencers, end users, or affected parties. Return only the JSON array.
"""

    @classmethod
    def get_technical_requirements_prompt(cls, user_input: str, domain_context: str = "unknown", objectives_context: str = "", constraints_context: str = "") -> str:
        """Generate focused prompt for extracting technical requirements only"""
        return f"""
FOCUSED TASK: Extract ONLY the technical requirements from this user request.

USER REQUEST: "{user_input}"
DOMAIN CONTEXT: {domain_context}
OBJECTIVES CONTEXT: {objectives_context}
CONSTRAINTS CONTEXT: {constraints_context}

Return JSON array of technical requirements with:
- id: unique identifier
...
Focus ONLY on technical needs, integrations, or specifications. Return only the JSON array.
"""

    @classmethod
    def get_assumptions_prompt(cls, user_input: str, domain_context: str = "unknown", full_context: str = "") -> str:
        """Generate focused prompt for extracting assumptions only"""
        return f"""
FOCUSED TASK: Extract ONLY the assumptions from this user request.

USER REQUEST: "{user_input}"
DOMAIN CONTEXT: {domain_context}
FULL CONTEXT: {full_context}

Return JSON array of assumptions with:
- id: unique identifier
- assumption: what is being assumed
- confidence: 0.0 to 1.0 confidence level
- impact_if_wrong: "critical"|"high"|"medium"|"low"
- validation_method: how to verify this assumption (optional)

Focus ONLY on what the user assumes to be true but might not be. Return only the JSON array.
"""

    @classmethod
    def get_context_prompt(cls, user_input: str, domain_context: str = "unknown") -> str:
        """Generate focused prompt for extracting context and metadata only"""
        return f"""
FOCUSED TASK: Extract ONLY the context and metadata from this user request.
...
- domain_context: refined domain context
- user_sentiment: "positive"|"neutral"|"negative"|"urgent"|"frustrated"|"excited"
- confidence_scores: object with confidence in analysis quality
- ambiguities: array of unclear elements
- clarifying_questions: array of questions to ask user
- implicit_requirements: array of inferred needs

Focus ONLY on context, sentiment, and analysis metadata. Return only the JSON object.
"""

    @classmethod
    def get_llm_query_prompt(cls, user_input: str, domain_context: str = "unknown") -> str:
        """
        DEPRECATED: Use focused prompts instead
